...
Rewrite of [text-to-svg](https://github.com/shrhdk/text-to-svg) in Typescript.
...
const options = { font, x: 0, y: 0, fontSize: 72, anchor: 'top', attributes };
...
Options is an optional object containing:
 ...
 - `anchor`: Anchor of object in coordinate. (default: `'left baseline'`)
   - (`left`, `center`, `right`) + (`baseline`, `top`, `middle`, `bottom`)
 - `attributes`: Key-Value pairs of attributes of `path` element.
 - `beforePath`: Content before the path.
 - `afterPath`: Content after the path.
...
string-to-svg depends on the following softwares. I thank great authors a lot.

- [text-to-svg](https://github.com/shrhdk/text-to-svg): Copytight (c) 2016 Hideki Shiro
